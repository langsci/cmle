...
is shown in Figure \ref{fig:grounded}. We see two QRIO robots and an environment consisting of objects of various 
...
direction of movement, etc., but these will always be very noisy (see Figure \ref{fig:png-perception}). 
\begin{figure}[htbp]
  \centerline{\includegraphics[width=.85\textwidth]{chap10/figs/png-perception}}
\caption{\small\label{fig:png-perception} 
...
Figure \ref{fig:prototypes-identifiers-words}, left-most column). Real
...
(see figure \ref{fig:prototypes-identifiers-words}). Internally, agents store a set of cases 
...
\caption{\small\label{fig:prototypes-identifiers-words} 
...
(see Figure \ref{fig:prototypes-identifiers-words}
...
introduced in the experiment (see Figure \ref{fig:png-results}, left). Apparently, agents are naming prototypical views of
...
the experimenter in Figure \ref{fig:png-perception} 
...
be rearranged to reflect that they are two views of the same object (see Figure \ref{fig:png-optimisation}). 
...
Figure \ref{fig:png-results}, right shows what happens when the robots use this heuristic. The population
exhibits still the same capacity to achieve communicative success as
in Figure \ref{fig:png-results}, left, but now the number of individuals
...
University in Berlin (see Figure \ref{fig:myon-action}).\cite{Steels:2012b}
...
although it could also be represented using the spike diagram used in the Proper Naming Game (figure \ref{vision}, right). 
...
In one of them, the robot stands before the mirror (Figure \ref{robot-mirror}) and observes its own 
...
The network used here is shown in Figure \ref{network}. Compared to the Proper Naming Game, we see the same relation between 
...
of light changes and is different for speaker and hearer (see Figure \ref{f:scene-example}). This difference did not 
...
performed language game experiments both for the same set-up as shown in Figure \ref{fig:grounded}, 
...
in psychological experiments (see Figure \ref{fig:munsell}). The color space used by the agents was the CIE L*a*b* space, where 
...
\end{verbatim}
A graphical representation of this network is shown in Figure \ref{fig:basic-strat}. 
\begin{figure}[htbp]
...
performed. The network for this strategy is shown in Figure \ref{fig:modi}. 
%http://www.workwithcolor.com/red-orange-color-hue-range-01.htm
\begin{figure}[htbp]
...
and expanded by three types of constructions (see for example Figure \ref{fig:syntactic-graded}, left, which is an instantiation of 
the Basic Color strategy in Figure \ref{fig:basic-strat}). 
...
word-string itself as the form. It is the unit called Yellow-unit in Figure \ref{fig:syntactic-graded}. 
...
the color-category is used in the filter-by-color-lenient operation (see Figure \ref{fig:basic-strat}). 
The new unit is called color-unit in Figure \ref{fig:syntactic-graded}, left. 
...
The phrase usually has additional semantic operations as well. Figure \ref{fig:syntactic-graded},left shows an example. 
...
Figure \ref{fig:syntactic-graded}, right shows another example, namely the structure for an instantiation of the 
...
the impact of embodiment on language. The experiment compares three conditions (see Figure \ref{fig:grounding-comparison}): 
...
figure \ref{fig:modi}) for four experimental conditions: 
\begin{enumerate}
