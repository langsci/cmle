...
robotic experiments (i) how these strategies are effective to {\it acquire} an existing language system in a 
population, (ii) how they are effective to {\it extend} a language system so that it remains adaptive 
to the needs and environments of its user, and (iii) how a population can {\it invent and coordinate} a new language system from 
...
evolution}
\begin{itemize}
\item {\it Language emergence} is the phenomenon whereby a language system, i.e. a set of conventions and conceptualizations
...
\item {\it Language change} happens when the components of a language system and its conceptualizations shift, but the 
...
\item {\it Language evolution} occurs when new strategies arise. There is plenty of evidence from creole languages and 
...
{\it emergence}: A large-scale macroscopic structure emerges from the individual activities of the agents (i.e. animals)
...
Living organisms are {\it adaptive} to their environment and so even within 
...
Nevertheless true {\it evolution} is only said to happen if there are new innate 
...
This is the idea behind the {\it recruitment theory} of language evolution, which postulates that cognition and 
...
So this hypothesis implies {\it culture-driven gene evolution} rather than gene-driven cultural 
...
\begin{enumerate}
\item There is a process of {\it generating} possible variants, often by making changes to 
existing variants or by information transfer between variants, for example through cross-over. 
\item There is a separate process of {\it testing} whether these variants satisfy 
...
\item There is a {\it self-enforcing causal loop} between the 
...
Clearly, the bottom line is that language users want to achieve {\bf communicative success}. 
...
{\bf Expressive adequacy}: The language systems available to the speaker and the hearer must have 
...
{\bf Cognitive effort}: In order to cope with the incredible speed of normal language production and comprehension, 
...
{\bf Learnability}: Speakers must occasionally expand their language systems to express novel meanings or 
...
{\bf Social conformity}: Speakers and hearers can greatly optimize their language production and 
...
thanks to its physiological and behavioral strategies its ecological {\it niche}. A niche is defined through the ecological conditions
...
a process called {\it niche construction}.\cite{Odling-Smee:2003} \is{fitness landscape}

Niche analysis is performed by defining a multi-dimensional space, further called the {\it niche space}. Each 
dimension or {\it niche factor} has to be dealt with by the species in order to remain viable. There 
...
{\bf Fitness landscapes of language systems}\\
...
at a critical point to preserve the disambiguation power of the system. See Figure \ref{fig:german}. 
\begin{figure}[htb!]
\centerline{
\includegraphics[width=0.75\textwidth]{chap12/figs/german-system.pdf}
}
\caption{\footnotesize{The German article system has morphological marking for case (nominative, accusative, dative, 
...
fitness landscape: 
\begin{enumerate}
\item {\it Search effort} which is the amount of search that is needed, scaled between minimum (when there is one marker for every of 
the 18 possible configurations and so no search is needed) and maximum (when there are no markers at all).
\item {\it Disambiguation power} which is based on the number of possible interpretations left after application of the grammar. 
...
\item {\it Articulatory effort} which is calculated in terms of the number of movements that articulators such as the lips and 
tongue have to make when pronouncing the different phonemes that make up each article. 
\item {\it Acoustic precision} which is based on the phonological distance between words.
\end{enumerate}
...
and processing effort, thus being an advantage for the hearer but not for the speaker (see Figure \ref{fig:german-system}). 
...
\caption{\footnotesize{Fitness landscape for the German marker system. When the number of markers increases (x-axis), the disambiguation 
...
(see Figure \ref{fig:spatial}) and uses a 
...
\caption{\footnotesize Set-up for the spatial language game experiments. Two robots are looking at a scene 
with a few blocks, a bigger box with tags indicating front, back or left and right side, and a global landmark in the form 
of a large tag pasted on the wall. {\it Top left and right:} World as seen through the camera of the robot. 
{\it Bottom left and right:} The internal world model as perceived by the left and right robot.}
\label{fig:spatial}
\end{figure}
\begin{enumerate}
\item The use of {\it projective relations} such as front/back, above/below, left/right. This requires imposing a coordinate
...
\item The use of {\it proximal relations} such as far/near. This requires that the distances of objects with respect to the 
reference point are computed and categorized. 
\item The use of {\it absolute relations}, such as North/West/East/South, related to global directions. 
\item The use of an {\it allocentric landmark} (as opposed to an egocentric one)
...
\caption{\footnotesize Investigation how the two language systems (lexical or grammatical) are performing with respect to the 
...
\begin{enumerate}
\item {\it Communicative success}, including whether production failed, interpretation failed, or the hearer pointed to the wrong 
object (pragmatic failure). We see (Figure \ref{fig:why-grammar-german}) that the more complex a situation becomes, the 
more grammar becomes relevant. 
\item {\it Semantic ambiguity}, being the number of semantic structures that remain after syntactic analysis and have to be mapped 
onto the perceived world in order to be disambiguated (See Figure \ref{fig:ambiguity}, left). Without grammar there is a lot 
more semantic ambiguity and hence more cognitive effort to consult the world model. 
\item {\it Interpretation sharing}, which takes place when the semantic structure derived by the hearer is equal to that originally 
intended by the speaker (See Figure \ref{fig:ambiguity}, right). Interpretation sharing is important in cases of displaced 
...
\caption{\footnotesize Left: performance for semantic ambiguity for the same three ecological conditions. We see that without grammar
...
{\bf The fitness landscape of language strategies}\\
...
\begin{enumerate}
\item The {\it brightness-only strategy} which uses only the L* dimension (dark, grey, bright, etc.).
\item The {\it hue-only strategy} which uses only the hue dimension (blue, yellow, green, etc.). 
\item And the {\it full-color strategy} which uses all dimensions. 
\end{enumerate}
...
\caption{\footnotesize{Niche analysis for three different 
...
is more optimal (see Figure \ref{fig:bar}). Not surprisingly, a hue-only strategy fails completely when the samples in each  
...
\caption{\footnotesize{Agents have been given a grammatical strategy that introduces constructions to signal 
...
A series of language games were played using contexts (such as the one shown in Figure \ref{fig:gramdev3}, left)
...
and semantic sharing). Experimental results are shown in Figure \ref{fig:gramdev3}. They confirm that as 
...
but also strategies need to have a score. These {\it strategy scores} should be updated using the same 
...
the score of the strategy can be updated as well (Figure \ref{fig:tagging}). This is necessary  
...
\begin{figure}[htb!]
	\begin{center}
		...
		\caption{\footnotesize{Different words are tagged with the strategy with which they 
...
to self-organize a colour lexicon from scratch. Figure \ref{fig:full-cs-dynamics} illustrates (on the left) how 
...
\caption{\footnotesize{The {\it Brightness Colour Strategy} also allows a population of agents (in this case 10)
...
\caption{\footnotesize{Communicative success stays constant. The Brightness Colour Strategy becomes 
...
Rich and complex dynamics results from this general system. In the results shown in Figure \ref{fig:strategies-one}, the 
...
contexts that are presented), we observe several possibilities. In one simulation run (see Figure \ref{fig:strategies-one}), 
...
as attested in the history of English color terms (see Figure \ref{fig:strategies-dynamics}). 
...
\caption{\footnotesize{Initially the brightness strategy is dominant, but then the full-colour 
...
{\bf A Meta-strategy for Generating new Conceptualization Strategies}\\
...
\caption{\footnotesize Example of a conceptualization for the phrase ``near to the box". {\it Top:} 
...
near objects are to the box. {\it Bottom:} IRL constraint network for this conceptualization.
...
and scenes like the one shown in figure \ref{fig:spatial}. An example of a constraint network from this domain 
is shown in Figure \ref{fig:spatial-strategies}. It is the network for the phrase ``near the box". `Near' is a proximal 
...
A {\it conceptualization strategy} is an abstraction of such a concrete conceptualization network. 
...
For example, the network shown in Figure \ref{fig:spatial-strategies} can be made more abstract to handle many instances 
...
way in which the speaker agent finds possible conceptualizations, namely through a search process (Figure \ref{fig:tree}). 

\begin{figure}
\begin{centering}
\includegraphics[width=1\columnwidth]{chap12/figs/conceptualization.pdf}
\end{centering}
\caption{\footnotesize The meaning of an utterance is found through a search process, starting from an empty network, progressively more 
...
Often there is more than one solution. (There are already four solutions in the tree in Figure \ref{fig:tree} and more solutions 
...
\caption{\footnotesize This experiment combines strategy invention, alignment, and category development by each strategy. The top figure
...
Figure \ref{fig:all} shows an example experiment in which all the aspects discussed in this section are brought together. 
...
{\bf Meta-strategies for generating new lexico-grammatical strategies}\\ 
...
Generally speaking, a lexico-grammatical strategy has two components: {\it diagnostics} which monitor how 
language processing is going and {\it repairs} which add new 
constructions to the agent's inventory. 

For {\it lexical strategies}, the diagnostics consist in checking whether there is part 
...
\end{center}
\caption{\footnotesize Two examples of the meaning of words in terms of a set of component features whose weight can change 
over time.}
...
constellation of features that is only partially matching. Figure \ref{fig:weightchange} shows examples of how these features 
...
{\it Grammatical strategies} come in two types. There are first of all strategies that express additional meaning using 
...
agreement marker {\it m-} evolved from the classifier {\it mayi}, which means 'non-flesh food'. 
...
might be masculine in one language (German: {\it der Tisch}) but feminine in another (French: {\it la table}). 
\end{enumerate}
...
\centerline{ii) \includegraphics[scale=.4]{chap12/figs/uinbui.pdf}} % fig14-ii-uinbui
  \caption{\footnotesize {Performance of the phonological reduction strategy.} i) Average performance values
...
experiment. The marker {\it -uinbui} erodes progressively to {\it -u}. A truncated variant is typically present for 
a while in the population until a phase transition happens and it becomes dominant.}
  \label{f:erosion}
\end{figure}

Figure \ref{f:erosion} shows the outcome of a computer simulation of this phonological reduction strategy. An agreement system based on meaningful markers is emerging using the meaningful marker strategy. But after agents reach a stable level of performance
...
of the original markers. Figure \ref{f:erosion} top) shows that the average marker length is decreasing from an 
...
Figure \ref{f:erosion} bottom) shows a typical example how a marker (in this case ``-uinbui") erodes. At the beginning there is still 
